...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['device'], $_POST['price'], $_POST['model'], $_POST['time'], $_POST['location'], $_POST['date'], $_POST['phone'], $_POST['facility'])) {
    ...
    // Prepare statement
    $stmt = $conn->prepare("INSERT INTO pickupdetails (device, price, model, time, location, date, phone, facility) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }

    // Bind parameters
    $stmt->bind_param("ssssssss", $device, $price, $model, $time, $location, $date, $phone, $facility);
